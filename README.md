...
## 更新日志
>### 5.6.4
>扩充天选时刻默认忽略关键字；天选时刻金额识别优化；内容屏蔽新增两项：右侧关注和弹窗，下方广告；新增更新提示开关；新增打卡弹幕发送间隔设置；新增天选时刻附加信息；窗口大小改变时脚本窗口位置会随之改变，并保持相对位置不变；天选时刻数据推送新增额外信息；天选时刻检查到缺少粉丝牌的天选时可快捷购买并参加；样式上的一些小改进；新增eula；部分重构，减少不必要的请求，完善错误处理，运行效率和代码格式优化。
