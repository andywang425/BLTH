...
## 更新日志
>### 5.6.6.3
>1.优化了天选时刻的直播间号收集机制。2.修复自动送礼在完成一轮送礼后无法进行下一轮送礼的bug。3.自动送礼细节改进。4.不给20或40级粉丝牌的应援团签到。5.优化天选时刻的休眠机制，避免在休眠前额外检查一轮。6.修复了【自动发弹幕】弹幕缓存不正确的bug（建议点击一下清除弹幕缓存）。7.修复EULA中的一个错误链接。8.天选时刻：特别关心分组里的up不再会被取关。9.内容屏蔽新增【屏蔽天选时刻弹窗及图标】和【屏蔽大乱斗弹窗及进度条】。
