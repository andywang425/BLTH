...
## 更新日志
>### 5.6.5.2
>1.减少了天选时刻某个API的请求频率，降低被风控概率。2.修复了天选时刻房间号记录不正确导致检查天选时有重复的bug。3.修复在有背景直播间无法观看直播的bug；修复切换清晰度时直播卡死的bug。4.新增中奖消息推送方式：酷推和Server酱Turbo版。5.Css浏览器兼容，不使用margin-inline。6.天选时刻金额识别支持繁体数字。7.能填写多个数据的设置项在保存时会自动去重（部分特殊设置除外）。8.天选时刻新增数据获取方式：从自定义直播间列表获取天选时刻数据，即用户手动填写要检查的直播间。
