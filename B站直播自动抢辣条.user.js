...
newWindow.init();$(()=>{let loadInfo=(delay)=>{setTimeout(()=>{const W=typeof unsafeWindow==='undefined'?window:unsafeWindow;if(W.BilibiliLive===undefined||parseInt(W.BilibiliLive.UID)===0||isNaN(parseInt(W.BilibiliLive.UID))){loadInfo(1000);window.toast(`[${GM_info.script.name}]无配置信息`,'warning');MYDEBUG('无配置信息');}else{Live_info.room_id=W.BilibiliLive.ROOMID;Live_info.uid=W.BilibiliLive.UID;BAPI.live_user.get_info_in_room(Live_info.room_id).then((response)=>{MYDEBUG('InitData: API.live_user.get_info_in_room',response);Live_info.mobile_verify=response.data.info.mobile_verify;Live_info.identification=response.data.info.identification;});BAPI.gift.gift_config().then((response)=>{MYDEBUG('InitData: API.gift.gift_config',response);if(!!response.data&&$.isArray(response.data)){Live_info.gift_list=response.data;}else if(!!response.data.list&&$.isArray(response.data.list)){Live_info.gift_list=response.data.list;}else{Live_info.gift_list=[{"id":6,"price":1000},{"id":1,"price":100},{'id':30607,'price':5000}];window.toast('直播间礼物数据获取失败，使用备用数据','error');}});Live_info.bili_jct=BAPI.getCookie('bili_jct');Live_info.ruid=W.BilibiliLive.ANCHOR_UID;Live_info.rnd=W.BilibiliLive.RND;Live_info.visit_id=W.__statisObserver?W.__statisObserver.__visitId:'';MYDEBUG("Live_info",Live_info);init();}},delay);};loadInfo(1000);addStyle();});Array.prototype.remove=function(val){let index=this.indexOf(val);if(index>-1){this.splice(index,1);}};function addStyle(){$('head').append(`
        <style>
            ...
            `)}
function init(){const MY_API={CONFIG_DEFAULT:{AUTO_GIFT:false,AUTO_GIFT_ROOMID:"0",AUTO_GROUP_SIGN:true,AUTO_TREASUREBOX:true,CHECK_HOUR_ROOM_INTERVAL:120,COIN:false,COIN_NUMBER:0,COIN_TYPE:"COIN_DYN",COIN_UID:0,EXCLUDE_ROOMID:"0",FORCE_LOTTERY:false,FORCE_LIGHT:false,GIFT_LIMIT:1,GIFT_SEND_HOUR:23,GIFT_SEND_MINUTE:59,GIFT_INTERVAL:5,GIFT_METHOD:"GIFT_SEND_TIME",GIFT_SORT:false,IN_TIME_RELOAD_DISABLE:false,LIVE_SIGN:true,LOGIN:true,LITTLE_HEART:false,LIGHT_MEDALS:"0",LIGHT_METHOD:"LIGHT_WHITE",MAX_GIFT:99999,MAX_TAB:5,MOBILE_HEARTBEAT:true,RANDOM_DELAY:true,RANDOM_SEND_DANMU:0,RANDOM_SKIP:0,REMOVE_ELEMENT_2233:false,REMOVE_ELEMENT_july:false,REMOVE_ELEMENT_player:false,RND_DELAY_END:5,RND_DELAY_START:2,SEND_ALL_GIFT:false,SHARE:true,SILVER2COIN:false,SPARE_GIFT_ROOM:"0",SPARE_GIFT_UID:"0",STORM:false,STORM_MAX_COUNT:100,STORM_ONE_LIMIT:200,STORM_QUEUE_SIZE:3,TIME_AREA_DISABLE:true,TIME_AREA_END_H0UR:8,TIME_AREA_END_MINUTE:0,TIME_AREA_START_H0UR:2,TIME_AREA_START_MINUTE:0,TIME_RELOAD:60,WATCH:true,},CACHE_DEFAULT:{UNIQUE_CHECK:0,AUTO_GROUP_SIGH_TS:0,DailyReward_TS:0,LiveReward_TS:0,TreasureBox_TS:0,Silver2Coin_TS:0,Gift_TS:0,GiftInterval_TS:0,MobileHeartBeat_TS:0,LittleHeart_TS:0},CONFIG:{},CACHE:{},GIFT_COUNT:{COUNT:0,SILVER_COUNT:0,CLEAR_TS:0,},init:()=>{try{BAPI.setCommonArgs(Live_info.bili_jct);}catch(err){console.error(`[${NAME}]`,err);return;}
...
return p},newMessage:(version)=>{let newMsg=`${version}更新提示：\n
...
MY_API.saveConfig();});$("input:radio[name='GIFT_TYPE']").change(function(){let a=$("div[data-toggle='GIFT_INTERVAL'] input:radio").is(':checked');if(a==true){MY_API.CONFIG.GIFT_METHOD='GIFT_INTERVAL';}
...
if(MY_API.TreasureBox.DOM.div_timer)MY_API.TreasureBox.DOM.div_timer.hide();if(MY_API.TreasureBox.DOM.div_tip)MY_API.TreasureBox.DOM.div_tip.html(htmltext);},getAward:async(captcha,cnt=0)=>{if(!MY_API.CONFIG.AUTO_TREASUREBOX)return $.Deferred().reject();if(cnt>3)return $.Deferred().resolve();return BAPI.TreasureBox.getAward(MY_API.TreasureBox.time_start,MY_API.TreasureBox.time_end,captcha).then((response)=>{MYDEBUG('TreasureBox.getAward: getAward',response);switch(response.code){case 0:window.toast(`[自动领取瓜子]领取了 ${response.data.awardSilver} 银瓜子`,'success');case-903:return $.Deferred().resolve();case-902:case-901:case-10017:await sleep(150);return MY_API.TreasureBox.captcha.calc().then((captcha)=>{return MY_API.TreasureBox.getAward(captcha,cnt);});case-800:MY_API.TreasureBox.setMsg('未绑定<br>手机');window.toast('[自动领取瓜子]未绑定手机，已停止','caution');return $.Deferred().reject();case-500:const p=$.Deferred();setTimeout(()=>{MY_API.TreasureBox.captcha.calc().then((captcha)=>{MY_API.TreasureBox.getAward(captcha,cnt+1).then(()=>p.resolve(),()=>p.reject());},()=>p.reject());},3e3);return p;case 400:if(response.msg.indexOf('拒绝')>-1){Live_info.blocked=true;MY_API.TreasureBox.setMsg('拒绝<br>访问');window.toast('[自动领取瓜子]访问被拒绝，您的帐号可能已经被关小黑屋，已停止','error');return $.Deferred().reject();}
...
await MY_API.Gift.getMedalList();let medal_list=MY_API.Gift.medal_list;MYDEBUG('Gift.run: Gift.getMedalList().then: Gift.medal_list',medal_list);if(medal_list&&medal_list.length>0){medal_list=medal_list.filter(it=>it.day_limit-it.today_feed>0&&it.level<20);medal_list=MY_API.Gift.sort_medals(medal_list);if(MY_API.CONFIG.EXCLUDE_ROOMID&&MY_API.CONFIG.EXCLUDE_ROOMID.length>0){ArrayEXCLUDE_ROOMID=MY_API.CONFIG.EXCLUDE_ROOMID.split(",");medal_list=medal_list.filter(Er=>ArrayEXCLUDE_ROOMID.findIndex(exp=>exp==Er.roomid)==-1);};await MY_API.Gift.auto_light(medal_list);let limit=MY_API.CONFIG.GIFT_LIMIT*86400;for(let v of medal_list){let response=await BAPI.room.room_init(parseInt(v.roomid,10));MY_API.Gift.room_id=parseInt(response.data.room_id,10);MY_API.Gift.ruid=v.target_id;MY_API.Gift.remain_feed=v.day_limit-v.today_feed;if(MY_API.Gift.remain_feed>0){await MY_API.Gift.getBagList();let now=ts_s();if(!MY_API.CONFIG.SEND_ALL_GIFT){let pass=MY_API.Gift.bag_list.filter(r=>MY_API.Gift.sendGiftList.includes(r.gift_id)&&r.gift_num>0&&r.expire_at>now&&(r.expire_at-now<limit));if(pass.length==0){break;}};if(MY_API.Gift.remain_feed>0){window.toast(`[自动送礼]勋章[${v.medalName}] 今日亲密度未满[${v.today_feed}/${v.day_limit}]，预计需要[${MY_API.Gift.remain_feed}]送礼开始`,'info');await MY_API.Gift.sendGift(v);if(!MY_API.CONFIG.SEND_ALL_GIFT){let pass=MY_API.Gift.bag_list.filter(r=>MY_API.Gift.sendGiftList.includes(r.gift_id)&&r.gift_num>0&&r.expire_at>now&&(r.expire_at-now<limit));if(pass.length==0){break;}else{MY_API.Gift.bag_list=pass;}}}else{window.toast(`[自动送礼]勋章[${v.medalName}] 今日亲密度已满`,'info');}}}}
...
SEND_GIFT_NOW=false;nextTimeDebug();return $.Deferred().resolve();},sendGift:async(medal)=>{await MY_API.Gift.getBagList();if(MY_API.Gift.remain_feed<=0){window.toast(`[自动送礼]勋章[${medal.medalName}] 送礼结束，今日亲密度已满[${medal.today_feed}/${medal.day_limit}]`,'info');return $.Deferred().resolve();}
if(MY_API.Gift.time<=0)MY_API.Gift.time=ts_s();if(!MY_API.CONFIG.SEND_ALL_GIFT){let pass=MY_API.Gift.bag_list.filter(r=>MY_API.Gift.sendGiftList.includes(r.gift_id)&&r.gift_num>0&&r.expire_at>MY_API.Gift.time&&(r.expire_at-MY_API.Gift.time<MY_API.CONFIG.GIFT_LIMIT*86400)&&r.corner_mark!='永久');if(pass.length==0){MY_API.Gift.over=true;return;}else{MY_API.bag_list=pass;}}else{let pass=MY_API.Gift.bag_list.filter(r=>r.gift_num>0&&r.corner_mark!='永久');if(pass.length==0){MY_API.Gift.over=true;return;}else{MY_API.bag_list=pass;}}
for(const v of MY_API.Gift.bag_list){if(medal.day_limit-medal.today_feed<=0){window.toast(`[自动送礼]勋章[${medal.medalName}] 送礼结束，今日亲密度已满[${medal.today_feed}/${medal.day_limit}]`,'info');return;}
const feed=MY_API.Gift.getFeedByGiftID(v.gift_id);if(feed>0){let feed_num=Math.floor(MY_API.Gift.remain_feed/feed);if(feed_num>v.gift_num)feed_num=v.gift_num;if(feed_num>0){return BAPI.gift.bag_send(Live_info.uid,v.gift_id,MY_API.Gift.ruid,feed_num,v.bag_id,MY_API.Gift.room_id,Live_info.rnd).then((response)=>{MYDEBUG('Gift.sendGift: API.gift.bag_send',response);if(response.code===0){v.gift_num-=feed_num;medal.today_feed+=feed_num*feed;MY_API.Gift.remain_feed-=feed_num*feed;window.toast(`[自动送礼]勋章[${medal.medalName}] 送礼成功，送出${feed_num}个${v.gift_name}，[${medal.today_feed}/${medal.day_limit}]距离升级还需[${MY_API.Gift.remain_feed}]`,'success');}else{window.toast(`[自动送礼]勋章[${medal.medalName}] 送礼异常:${response.msg}`,'caution');}},()=>{window.toast('[自动送礼]包裹送礼失败，请检查网络','error');return delayCall(()=>MY_API.Gift.sendGift(medal));});}}}}},MobileHeartBeat:{run:async()=>{if(MY_API.CONFIG.MOBILE_HEARTBEAT==false)return $.Deferred().resolve();if(!checkNewDay(MY_API.CACHE.MobileHeartBeat_TS)){runMidnight(MY_API.MobileHeartBeat.run,'移动端心跳');return $.Deferred().resolve();}
...
stormInterval=setInterval(()=>process(),MY_API.CONFIG.STORM_ONE_LIMIT);return $.Deferred().resolve();}},LITTLE_HEART:{medalRoom_list:[],liveRoom_list:[],getMedalRoomList:(page=1)=>{if(page===1)MY_API.LITTLE_HEART.medalRoom_list=[];return BAPI.i.medal(page,25).then((response)=>{MYDEBUG('LITTLE_HEART.getMedalRoomList: API.i.medal',response);for(let i of response.data.fansMedalList){MY_API.LITTLE_HEART.medalRoom_list=MY_API.LITTLE_HEART.medalRoom_list.concat(i.roomid);}
MYDEBUG('MY_API.LITTLE_HEART.medalRoom_list',MY_API.LITTLE_HEART.medalRoom_list)
if(response.data.pageinfo.curPage<response.data.pageinfo.totalpages)MY_API.LITTLE_HEART.getMedalRoomList(page+1);},()=>{window.toast('[小心心]获取勋章列表失败，请检查网络','error');return delayCall(()=>MY_API.LITTLE_HEART.getMedalRoomList(page));});},checkRoomList:async(roomList)=>{let returnRoom=[];for(let r of roomList){await BAPI.room.get_info(r).then((re)=>{if(re.data.live_status===1){returnRoom=returnRoom.concat(re.data.room_id);return}},()=>{window.toast('[小心心]获取房间信息失败，请检查网络','error');return delayCall(()=>MY_API.LITTLE_HEART.checkRoomList(roomList));});};MYDEBUG('MY_API.LITTLE_HEART.checkRoomList returnRoom',returnRoom)
if(returnRoom!=undefined){return returnRoom}else{window.toast('[小心心]当前粉丝勋章列表中无正在直播房间，5分钟后重试');return delayCall(()=>MY_API.LITTLE_HEART.getMedalRoomList(page),300e3);}},checkRoom:async(roomid)=>{let roomCheck=undefined;await BAPI.room.get_info(roomid).then((re)=>{if(re.data.live_status===1){roomCheck=true;}else{roomCheck=false;}},()=>{window.toast('[小心心]获取房间信息失败，请检查网络','error');return delayCall(()=>MY_API.LITTLE_HEART.checkRoomList(roomList));});return roomCheck;},doHeartBeat:async(roomId)=>{if(liveRoom_list.indexOf(roomId)==-1)return
window.toast(`[小心心]在新标签页中打开房间${roomId}【请勿点击】`,'info');MYDEBUG('[小心心]打开标签页房间',liveRoom_list);let openInTabCallBcak=await GM_openInTab(`https://live.bilibili.com/${String(roomId)}`,'loadInBackground');openInTabCallBcak_list.push(openInTabCallBcak);let tabTimer=setInterval(async()=>{;if(openInTabCallBcak!=undefined)openInTabCallBcak.close();if(!checkNewDay(MY_API.CACHE.LittleHeart_TS)){clearInterval(tabTimer);MYDEBUG('[小心心]',`今日任务完成，房间${roomId}不再打开`);return}
if(MY_API.LITTLE_HEART.checkRoom(roomId)){openInTabCallBcak=await GM_openInTab(`https://live.bilibili.com/${String(roomId)}`,'loadInBackground');}else{window.toast(`[小心心]房间${roomId}下播，即将再次寻找开播房间`,'info');clearInterval(tabTimer);delListItem(roomId,liveRoom_list);delListItem(openInTabCallBcak,openInTabCallBcak_list)
return MY_API.LITTLE_HEART.run();}},310e3);return},checkHeart:()=>{const checkbag=()=>{BAPI.gift.bag_list().then((re)=>{MYDEBUG('[小心心]检查包裹',re)
let heart=re.data.list.filter(r=>r.gift_id==30607&&r.gift_num==24&&r.corner_mark=="7天");MYDEBUG('[小心心]包裹内7天24个小心心',heart);if(heart.length>0){clearInterval(timer);window.toast('[小心心]今日小心心已全部获取','success');MY_API.CACHE.LittleHeart_TS=ts_ms();MY_API.saveCache();MYDEBUG('[小心心]',openInTabCallBcak_list)
closeAllTab(openInTabCallBcak_list);return}})}
checkbag();let timer=setInterval(()=>{checkbag();},300e3);return},run:async()=>{if(!MY_API.CONFIG.LITTLE_HEART)return $.Deferred().resolve();if(!checkNewDay(MY_API.CACHE.LittleHeart_TS)){return $.Deferred().resolve();}
await MY_API.LITTLE_HEART.getMedalRoomList();let room_list=await MY_API.LITTLE_HEART.checkRoomList(MY_API.LITTLE_HEART.medalRoom_list);liveRoom_list=room_list.filter(r=>!liveRoom_list.includes(r));let tabNum=liveRoom_list.length;if(!tabNum)return
for(let room of liveRoom_list){if(tabNum<MY_API.CONFIG.MAX_TAB){await MY_API.LITTLE_HEART.doHeartBeat(room);tabNum++;}else{break;}};MY_API.LITTLE_HEART.checkHeart();return}}};MY_API.init().then(()=>{try{const promiseInit=$.Deferred();const uniqueCheck=()=>{const t=Date.now();if(t-MY_API.CACHE.UNIQUE_CHECK>=0&&t-MY_API.CACHE.UNIQUE_CHECK<=15e3){$('.link-toast').hide();$('.igiftMsg').hide();MY_API.CONFIG.AUTO_TREASUREBOX=false;window.toast('有其他直播间页面的脚本正在运行，本页面脚本停止运行','caution');return promiseInit.reject();}else{return promiseInit.resolve();}};uniqueCheck().then(()=>{let timer_unique;const uniqueMark=()=>{timer_unique=setTimeout(uniqueMark,10e3);MY_API.CACHE.UNIQUE_CHECK=Date.now();MY_API.saveCache(false);};window.addEventListener('unload',()=>{if(timer_unique){clearTimeout(timer_unique);MY_API.CACHE.UNIQUE_CHECK=0;MY_API.saveCache();}
closeAllTab(openInTabCallBcak_list);});uniqueMark();if(parseInt(Live_info.uid)===0||isNaN(parseInt(Live_info.uid))){MY_API.chatLog('未登录，请先登录再使用脚本','warning');return};MY_API.newMessage(GM_info.script.version);MYDEBUG('MY_API.CONFIG',MY_API.CONFIG);StartPlunder(MY_API);})}
...
function delListItem(item,list){list.splice(list.indexOf(item),1)}
function closeAllTab(tabCallBack_list){if(tabCallBack_list!=undefined){for(let tab of tabCallBack_list){tab.close();}}
return}
