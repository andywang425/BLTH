...
try{if(!MY_API.CONFIG.AUTO_GIFT)return $.Deferred().resolve();if(!isTime(MY_API.CONFIG.GIFT_SEND_HOUR,MY_API.CONFIG.GIFT_SEND_MINUTE)&&SEND_GIFT_NOW==false){let alternateTime=getIntervalTime(MY_API.CONFIG.GIFT_SEND_HOUR,MY_API.CONFIG.GIFT_SEND_MINUTE);setTimeout(MY_API.Gift.run,alternateTime);let runTime=new Date(ts_ms()+alternateTime).toLocaleString();MYDEBUG("[自动送礼]",`将在${runTime}进行自动送礼`);return $.Deferred().resolve();};MY_API.Gift.over=false
...
MY_API.CACHE.Gift_TS=ts_ms();MY_API.saveCache();}
...
SEND_GIFT_NOW=false;nextTimeDebug();},sendGift:async(medal)=>{await MY_API.Gift.getBagList();if(MY_API.Gift.remain_feed<=0){window.toast(`[自动送礼]勋章[${medal.medalName}] 送礼结束，今日亲密度已满[${medal.today_feed}/${medal.day_limit}]`,'info');return $.Deferred().resolve();}
