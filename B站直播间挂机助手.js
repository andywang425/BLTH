    ...
    const MY_API = {
      ...
      creatSetBox: async () => {
        ...
        const saveAction = (div) => {
          ...
          MY_API.CONFIG.DANMU_MODIFY_REGEX = val;
          // DANMU_MODIFY_UID
          ...
          // DANMU_MODIFY_POOL
          val = parseInt(div.find('div[data-toggle="DANMU_MODIFY_POOL"] .str').val());
          if (isNaN(val)) return window.toast('[弹幕池] 错误输入', 'caution');
          MY_API.CONFIG.DANMU_MODIFY_POOL = val;
          // DANMU_MODIFY_COLOR
          val = div.find('div[data-toggle="DANMU_MODIFY_COLOR"] .str').val();
          MY_API.CONFIG.DANMU_MODIFY_COLOR = val;
          // DANMU_MODIFY_SIZE
          val = parseFloat(div.find('div[data-toggle="DANMU_MODIFY_SIZE"] .str').val());
          if (isNaN(val) || val < 0) return window.toast('[弹幕大小] 错误输入', 'caution');
          MY_API.CONFIG.DANMU_MODIFY_SIZE = val;
      ...
      DANMU_MODIFY: {
        check: (info) => {
          let regex,
            uid = info[2][0],
            danmu = info[1];
          try { regex = eval(MY_API.CONFIG.DANMU_MODIFY_REGEX) }
          catch (e) { MYDEBUG('bliveproxy', `正则表达式出错 ${MY_API.CONFIG.DANMU_MODIFY_REGEX}`); regex = /^【/; }
          if (regex.test(danmu) || findVal(MY_API.CONFIG.DANMU_MODIFY_UID, uid) > -1) return true;
          else return false;
        },
        run: () => {
          bliveproxy.addCommandHandler('DANMU_MSG', command => {
            ...
            if (!MY_API.DANMU_MODIFY.check(info)) return $.Deferred().resolve();
            // 显示模式
            info[0][1] = MY_API.CONFIG.DANMU_MODIFY_POOL;
            // 尺寸
            info[0][2] *= MY_API.CONFIG.DANMU_MODIFY_SIZE;
            // 颜色
            info[0][3] = Number("0x" + MY_API.CONFIG.DANMU_MODIFY_COLOR.replace("#", ""));
          })
        }
      }
    };
