...
// @require        https://cdn.jsdelivr.net/gh/andywang425/BLTH@390ab3b0d13c8125432dc58257f0b3def57e1519/assets/js/library/BilibiliAPI_Mod.min.js
...
// @resource       main     https://cdn.jsdelivr.net/gh/andywang425/BLTH@390ab3b0d13c8125432dc58257f0b3def57e1519/assets/html/main.min.html
    ...
    const MY_API = {
      ...
      PLATE_ACTIVITY: {
        ...
        getLotteryMyTimes: (obj) => {
          return BAPI.x.activity.getLotteryMyTimes(obj.sid).then(response => {
            ...
            switch (response.code) {
              ...
              case 75003:
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】活动已结束`, 'warning');
                return -1;
              case 75001:
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】活动不存在`, 'warning');
                return -1;
              case 412:
              case 'NET_ERR':
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】获取抽奖次数API被风控，30分钟后重试<br>${response.message}`);
                return delayCall(() => MY_API.PLATE_ACTIVITY.getLotteryMyTimes(obj), 1800e3);
              default:
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】获取剩余抽奖次数出错<br>${response.message}`, 'success');
                return -1;
            }
          })
        },
          ...
          return BAPI.x.activity.addLotteryTimes(obj.sid).then(response => {
            ...
            switch (response.code) {
              ...
              case 75001:
                ...
                return 2;
              case 75405:
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】获取抽奖次数以达到上限`, 'info');
                return 1;
              case 412:
              case 'NET_ERR':
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】增加抽奖次数API被风控，30分钟后重试<br>${response.message}`);
                return delayCall(() => MY_API.PLATE_ACTIVITY.addLotteryTimes(obj), 1800e3);
              default:
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】增加抽奖次数出错<br>${response.message}`, 'error');
                return 2;
            }
          })
        },
            ...
            switch (response.code) {
              case 0:
                ...
                if (giftName.includes('未中奖'))
                  MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】未中奖`, 'info');
                else
                  MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】中奖<br>奖品：${giftName}`, 'info');
                return true;
              case 75003:
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】活动已结束`, 'warning');
                return false;
              case 75001:
                ...
                return false;
              case 75415:
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】抽奖次数已用尽`, 'warning');
                return false;
              case 75400:
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】抽奖速度过快，10秒后重试`, 'warning');
                return delayCall(() => MY_API.PLATE_ACTIVITY.doLottery(obj), 10e3);
              case 412:
              case 'NET_ERR':
                MY_API.chatLog(`[转盘抽奖] 转盘【${obj.name}】参与抽奖API被风控，30分钟后重试<br>${response.message}`);
                return delayCall(() => MY_API.PLATE_ACTIVITY.doLottery(obj), 1800e3);
              default:
        ...
        run: async () => {
          ...
          if (MY_API.CACHE.PlateActivity_TS) {
          ...
          for (const plate of MY_API.PLATE_ACTIVITY.plateData) {
            ...
            for (let i = 0; i < MY_API.PLATE_ACTIVITY.maxTimes; i++) {
              ...
              switch (code) {
                case 0: continue; // 继续获取次数
                case 1: break; // 停止获取次数
                case 2: nextFlag = true; break; // 跳到下一个抽奖
                default: break;
              }
            }
            if (nextFlag) continue;
            lotteryTimes = await MY_API.PLATE_ACTIVITY.getLotteryMyTimes(plate);
            if (lotteryTimes <= 0) continue;
            MY_API.chatLog(`[转盘抽奖] 转盘【${plate.name}】共有${lotteryTimes}次抽奖机会，开始抽奖`, 'info');
            for (let i = 0; i < MY_API.PLATE_ACTIVITY.maxTimes; i++) {
              let bool = await MY_API.PLATE_ACTIVITY.doLottery(plate);
          ...
          return setTimeout(() => MY_API.PLATE_ACTIVITY.run(), MY_API.CONFIG.PLATE_ACTIVITY_CHECK_INTERVAL * 60e3);
        }
      }
    };
    MY_API.init().then(() => {
