...
return p},newMessage:(version)=>{try{const cache=localStorage.getItem(`${NAME}_NEWMSG_CACHE`);if((cache==undefined||cache==null||cache!='4.4.1.2')&&version=='4.4.1.2'){const linkMsg=(msg,link)=>{return'<a href=\"'+link+'\"target=\"_blank\">'+msg+'</a>';};layer.open({title:`${version}更新提示`,content:`1.修复短号房间弹幕发送异常的问题<br>
                            2.修复应援团签到bug<br>
...
MY_API.saveConfig();});},end:()=>{msgHide='hide';localStorage.setItem(`${NAME}_msgHide`,msgHide);$('.igiftMsg').hide();$('.link-toast').hide();ct.animate({scrollTop:0},0);setTimeout(()=>{ct.animate({scrollTop:ct.prop("scrollHeight")},10)},100);document.getElementById('hiderbtn').innerHTML="显示窗口和提示信息";}});};btn.click(()=>{if(hideBtnClickable){hideBtnClickable=false;setTimeout(function(){hideBtnClickable=true},200);if(msgHide=='show'){msgHide='hide';localStorage.setItem(`${NAME}_msgHide`,msgHide);$('.igiftMsg').hide();$('.link-toast').hide();layer.close(mainIndex);ct.animate({scrollTop:0},0);setTimeout(()=>{ct.animate({scrollTop:ct.prop("scrollHeight")},10)},100);document.getElementById('hiderbtn').innerHTML="显示窗口和提示信息";}
else{msgHide='show';localStorage.setItem(`${NAME}_msgHide`,msgHide);$('.igiftMsg').show();$('.link-toast').show();openMainWindow();ct.animate({scrollTop:ct.prop("scrollHeight")},0);document.getElementById('hiderbtn').innerHTML="隐藏窗口和提示信息";}}});$('.attention-btn-ctnr').append(btn);if(!MY_API.CACHE.DailyReward_TS){layer.tips('点我隐藏/显示控制面板','#hiderbtn',{tips:1})}
if(msgHide=='show'){openMainWindow()}},chatLog:function(text,type='info'){let div=$("<div class='igiftMsg'>");let msg=$("<div>");let ct=$('#chat-history-list');let myDate=new Date();msg.text(text);div.text(myDate.toLocaleString());div.append(msg);div.css({'text-align':'center','border-radius':'4px','min-height':'30px','width':'256px','color':'#9585FF','line-height':'30px','padding':'0 10px','margin':'10px auto',});msg.css({'word-wrap':'break-word','width':'100%','line-height':'1em','margin-bottom':'10px',});switch(type){case'warning':div.css({'border':'1px solid rgb(236, 221, 192)','color':'rgb(218, 142, 36)','background':'rgb(245, 235, 221) none repeat scroll 0% 0%',});break;case'success':div.css({'border':'1px solid rgba(22, 140, 0, 0.28)','color':'rgb(69, 171, 69)','background':'none 0% 0% repeat scroll rgba(16, 255, 0, 0.18)',});break;case'error':div.css({'border':'1px solid rgb(255, 46, 0)','color':'white','background':'none 0% 0% repeat scroll #ff4c4c',});break;default:div.css({'border':'1px solid rgb(203, 195, 255)','background':'rgb(233, 230, 255) none repeat scroll 0% 0%',});}
