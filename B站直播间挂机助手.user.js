...
// @name:en        B站直播间挂机助手
// @author         andywang425
// @description    自动获取小心心，直播区签到，应援团签到，银瓜子换硬币，完成主站每日任务(登录,观看视频,投币,分享视频)，批量送礼、点亮勋章，参与Bilibili直播区抽奖(现在极少)，参加被广播的节奏风暴(几乎没有)，定时发弹幕，快捷购买粉丝勋章
// @description:en 自动获取小心心，直播区签到，应援团签到，银瓜子换硬币，完成主站每日任务(登录,观看视频,投币,分享视频)，批量送礼、点亮勋章，参与Bilibili直播区抽奖(现在极少)，参加被广播的节奏风暴(几乎没有)，定时发弹幕，快捷购买粉丝勋章
...
return p},newMessage:(version)=>{try{const cache=localStorage.getItem(`${NAME}_NEWMSG_CACHE`);if((cache==undefined||cache==null||cache!='4.4.2')&&version=='4.4.2'){const linkMsg=(msg,link)=>{return'<a href=\"'+link+'\"target=\"_blank\">'+msg+'</a>';};layer.open({title:`${version}更新提示`,content:`1.修复送礼失败的问题<br>
                            2.优化了自动投币的检测机制<br>
...
MY_API.saveConfig();});$("input:radio[name='LIGHT_TYPE']").change(function(){if($("div[data-toggle='LIGHT_WHITE'] input:radio").is(':checked')){MY_API.CONFIG.LIGHT_METHOD='LIGHT_WHITE';}
else{MY_API.CONFIG.LIGHT_METHOD='LIGHT_BLACK';}
MY_API.saveConfig();});$("input:radio[name='GIFT_TYPE']").change(function(){if($("div[data-toggle='GIFT_INTERVAL'] input:radio").is(':checked')){MY_API.CONFIG.GIFT_METHOD='GIFT_INTERVAL';}
else{MY_API.CONFIG.GIFT_METHOD='GIFT_SEND_TIME';}
MY_API.saveConfig();});$("input:radio[name='GIFT_SORT']").change(function(){if($("div[data-toggle='GIFT_SORT_HIGH'] input:radio").is(':checked')){MY_API.CONFIG.GIFT_SORT='high';}
else{MY_API.CONFIG.GIFT_SORT='low';}
MY_API.saveConfig();});},end:()=>{msgHide='hide';localStorage.setItem(`${NAME}_msgHide`,msgHide);$('.igiftMsg').hide();$('.link-toast').hide();if($(lockBtn).attr('class')==='icon-item icon-font icon-lock-1 active')
$(lockBtn).trigger('click');else $(lockBtn).trigger('click').trigger('click');document.getElementById('hiderbtn').innerHTML="显示窗口和提示信息";}});};btn.click(()=>{if(hideBtnClickable){hideBtnClickable=false;setTimeout(function(){hideBtnClickable=true},200);if(msgHide=='show'){msgHide='hide';localStorage.setItem(`${NAME}_msgHide`,msgHide);$('.igiftMsg').hide();$('.link-toast').hide();layer.close(mainIndex);if($(lockBtn).attr('class')==='icon-item icon-font icon-lock-1 active')
$(lockBtn).trigger('click');else $(lockBtn).trigger('click').trigger('click');document.getElementById('hiderbtn').innerHTML="显示窗口和提示信息";}
else{msgHide='show';localStorage.setItem(`${NAME}_msgHide`,msgHide);$('.igiftMsg').show();$('.link-toast').show();openMainWindow();if($(lockBtn).attr('class')==='icon-item icon-font icon-lock-1 active')
$(lockBtn).trigger('click');else $(lockBtn).trigger('click').trigger('click');document.getElementById('hiderbtn').innerHTML="隐藏窗口和提示信息";}}});$('.attention-btn-ctnr').append(btn);if(!MY_API.CACHE.DailyReward_TS){layer.tips('点我隐藏/显示控制面板','#hiderbtn',{tips:1})}
if(msgHide=='show'){openMainWindow()}},chatLog:function(text,type='info'){let div=$("<div class='igiftMsg'>");let msg=$("<div>");let myDate=new Date();msg.text(text);div.text(myDate.toLocaleString());div.append(msg);div.css({'text-align':'center','border-radius':'4px','min-height':'30px','width':'256px','color':'#9585FF','line-height':'30px','padding':'0 10px','margin':'10px auto',});msg.css({'word-wrap':'break-word','width':'100%','line-height':'1em','margin-bottom':'10px',});switch(type){case'warning':div.css({'border':'1px solid rgb(236, 221, 192)','color':'rgb(218, 142, 36)','background':'rgb(245, 235, 221) none repeat scroll 0% 0%',});break;case'success':div.css({'border':'1px solid rgba(22, 140, 0, 0.28)','color':'rgb(69, 171, 69)','background':'none 0% 0% repeat scroll rgba(16, 255, 0, 0.18)',});break;case'error':div.css({'border':'1px solid rgb(255, 46, 0)','color':'white','background':'none 0% 0% repeat scroll #ff4c4c',});break;default:div.css({'border':'1px solid rgb(203, 195, 255)','background':'rgb(233, 230, 255) none repeat scroll 0% 0%',});}
if(msgHide=='show'){ct.find('#chat-items').append(div);}else{ct.find('#chat-items').append(div.hide());}
...
if(MY_API.Gift.time<=0)MY_API.Gift.time=ts_s();if(!MY_API.CONFIG.SEND_ALL_GIFT){let pass=MY_API.Gift.bag_list.filter(r=>MY_API.Gift.sendGiftList.includes(r.gift_id)&&r.gift_num>0&&r.corner_mark.substring(0,r.corner_mark.indexOf("天"))<=MY_API.CONFIG.GIFT_LIMIT);MYDEBUG("pass",pass)
...
if(count>=control){endFunc();break;}
else
await sleep(300*1000);}}}else{endFunc();}
return $.Deferred().resolve();}},AUTO_DANMU:{setValue:(array,index)=>{if(MY_API.CONFIG[array][index]===undefined&&index>0)
...
let reset=(delay)=>{let resetTimer=setTimeout(()=>{if(API.raffleId_list.length>0||API.guardId_list.length>0||API.pkId_list.length>0){MYDEBUG('[刷新直播间]','还有礼物没抽，延迟15s后刷新直播间');reset(15000);return}
if(checkNewDay(API.CACHE.LittleHeart_TS)){MYDEBUG('[刷新直播间]','正在获取小心心，10分钟后再次检查');clearTimeout(resetTimer);reset(600e3);return}
