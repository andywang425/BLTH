...
const a=$(`<div class="link-toast ${type} fixed"><span class="toast-text">${msg}</span></div>`)[0];document.body.appendChild(a);a.style.top=(document.body.scrollTop+list.length*40+10)+'px';a.style.left=(document.body.offsetWidth+document.body.scrollLeft-a.offsetWidth-5)+'px';if(msgHide=='hide'){$('.link-toast').hide();}
list.push(a);setTimeout(()=>{a.className+=' out';setTimeout(()=>{list.shift();list.forEach((v)=>{v.style.top=(parseInt(v.style.top,10)-40)+'px';});$(a).remove();},200);},timeout);};return $.Deferred().resolve();}catch(err){console.error(`初始化浮动提示时出现异常`,err);return $.Deferred().reject();}}}},addStyle=async()=>{const layerCss=await GM_getResourceText('layerCss');const myCss=".igiftMsg_input{outline:none;border:1px solid #e9eaec;background-color:#fff;border-radius:4px;padding:1px 0 0;overflow:hidden;font-size:12px;line-height:19px;width:30px;z-index:'10001';}.igiftMsg_btn{background-color:#23ade5;color:#fff;border-radius:4px;border:none;padding:5px;cursor:pointer;box-shadow:0 0 2px #00000075;line-height:10px;z-index:10001;}.igiftMsg_fs{border:2px solid #d4d4d4;z-index:10001;}";const AllCss=layerCss+myCss;return await GM_addStyle(AllCss);},ct=$('#chat-history-list'),lockBtn='span[title=\"弹幕滚动锁定\"]',getScrollPosition=(el=window)=>({x:el.pageXOffset!==undefined?el.pageXOffset:el.scrollLeft,y:el.pageYOffset!==undefined?el.pageYOffset:el.scrollTop});let msgHide=localStorage.getItem(`${NAME}_msgHide`)||'hide',gift_join_try=0,guard_join_try=0,pk_join_try=0,SEND_GIFT_NOW=false,SEND_DANMU_NOW=false,LIGHT_MEDAL_NOW=false,hideBtnClickable=true,Live_info={room_id:undefined,uid:undefined,ruid:undefined,tid:undefined,mobile_verify:undefined,gift_list:undefined,rnd:undefined,visit_id:undefined,identification:undefined,bili_jct:undefined},userToken=undefined,tokenData=JSON.parse(localStorage.getItem(`${NAME}_Token`))||{};newWindow.init();window.onload=()=>{const loadInfo=(delay)=>{setTimeout(async()=>{const W=typeof unsafeWindow==='undefined'?window:unsafeWindow;if((W.BilibiliLive===undefined||parseInt(W.BilibiliLive.UID)===0||isNaN(parseInt(W.BilibiliLive.UID)))){return loadInfo(1000);}else{Live_info.room_id=W.BilibiliLive.ROOMID;Live_info.uid=W.BilibiliLive.UID;Live_info.tid=W.BilibiliLive.ANCHOR_UID;await BAPI.live_user.get_info_in_room(Live_info.room_id).then((response)=>{MYDEBUG('InitData: API.live_user.get_info_in_room',response);Live_info.mobile_verify=response.data.info.mobile_verify;Live_info.identification=response.data.info.identification;});await BAPI.gift.gift_config().then((response)=>{MYDEBUG('InitData: API.gift.gift_config',response);if(!!response.data&&$.isArray(response.data)){Live_info.gift_list=response.data;}else if(!!response.data.list&&$.isArray(response.data.list)){Live_info.gift_list=response.data.list;}else{Live_info.gift_list=[{"id":6,"price":1000},{"id":1,"price":100},{'id':30607,'price':5000}];window.toast('直播间礼物数据获取失败，使用备用数据','error');}});Live_info.bili_jct=BAPI.getCookie('bili_jct');Live_info.ruid=W.BilibiliLive.ANCHOR_UID;Live_info.rnd=W.BilibiliLive.RND;Live_info.visit_id=W.__statisObserver?W.__statisObserver.__visitId:'';MYDEBUG("Live_info",Live_info);init();}},delay);};loadInfo(1000);addStyle();};Array.prototype.remove=function(val){const index=this.indexOf(val);if(index>-1){this.splice(index,1);}};function init(){const MY_API={CONFIG_DEFAULT:{AUTO_DANMU:false,AUTO_GIFT:false,AUTO_GIFT_ROOMID:["0"],AUTO_GROUP_SIGN:true,CHECK_HOUR_ROOM:false,CHECK_HOUR_ROOM_INTERVAL:600,COIN:false,COIN_NUMBER:0,COIN_TYPE:"COIN_DYN",COIN_UID:0,DANMU_CONTENT:["这是一条弹幕"],DANMU_ROOMID:["22474988"],DANMU_INTERVAL_TIME:["10m"],EXCLUDE_ROOMID:["0"],FORCE_LOTTERY:false,FORCE_LIGHT:false,GIFT_LIMIT:1,GIFT_SEND_HOUR:23,GIFT_SEND_MINUTE:59,GIFT_INTERVAL:5,GIFT_METHOD:"GIFT_SEND_TIME",GIFT_SORT:'high',IN_TIME_RELOAD_DISABLE:false,LOTTERY:true,LIVE_SIGN:true,LOGIN:true,LITTLE_HEART:true,LIGHT_MEDALS:["0"],LIGHT_METHOD:"LIGHT_WHITE",MAX_GIFT:99999,MATERIAL_LOTTERY:true,MATERIAL_LOTTERY_CHECK_INTERVAL:10,MATERIAL_LOTTERY_IGNORE_QUESTIONABLE_LOTTERY:true,NOSLEEP:true,RANDOM_DELAY:true,RANDOM_SEND_DANMU:0,RANDOM_SKIP:0,REMOVE_ELEMENT_2233:false,REMOVE_ELEMENT_activity:false,REMOVE_ELEMENT_rank:true,RND_DELAY_END:5,RND_DELAY_START:2,SEND_ALL_GIFT:false,SHARE:true,SILVER2COIN:false,SPARE_GIFT_ROOM:"0",STORM:false,STORM_MAX_COUNT:100,STORM_ONE_LIMIT:200,STORM_QUEUE_SIZE:3,TIME_AREA_DISABLE:true,TIME_AREA_END_H0UR:8,TIME_AREA_END_MINUTE:0,TIME_AREA_START_H0UR:2,TIME_AREA_START_MINUTE:0,TIME_RELOAD:false,TIME_RELOAD_MINUTE:120,WATCH:true,},CACHE_DEFAULT:{UNIQUE_CHECK:0,AUTO_SEND_DANMU_TS:[],AUTO_GROUP_SIGH_TS:0,DailyReward_TS:0,LiveReward_TS:0,Silver2Coin_TS:0,Gift_TS:0,GiftInterval_TS:0,LittleHeart_TS:0,materialobject_ts:0,last_aid:632,},CONFIG:{},CACHE:{},GIFT_COUNT:{COUNT:0,SILVER_COUNT:0,CLEAR_TS:0,},init:async()=>{const tabList=$('.tab-list.dp-flex'),ContributionList=$(tabList.children('li')[0]),GreatNavigation=$(tabList.children('li')[1]),tabContent=$('.tab-content'),tabOffSet=tabContent.offset(),tabHeight=tabContent.height(),ctWidth=ct.outerWidth(true),ctHeight=ct.height(),top=tabOffSet.top,left=tabOffSet.left,pic=$('.pic.pointer'),menuDiv=$(`<li data-v-2fdbecb2="" data-v-d2be050a="" class="item dp-i-block live-skin-separate-border border-box t-center pointer live-skin-normal-text" id = "menuDiv">脚本日志</li>`),eleList=['.chat-history-list','.attention-btn-ctnr','.live-player-mounter'];if(eleList.some(i=>i.length===0)||tabList.length===0||tabContent.length===0){window.toast('必要页面元素缺失，强制运行（可能会看不到控制面板，提示信息）','error');}
tabList.append(menuDiv);function hidePic(hide=true){if(hide)pic.children('img').hide()
else pic.children('img').show()};let menuIndex=await layer.open({type:1,title:false,offset:[String(top-getScrollPosition().y)+'px',left+'px'],closeBtn:0,shade:0,zIndex:9999,fixed:false,area:[ctWidth+'px',String(ctHeight+tabHeight)+'px'],anim:-1,isOutAnim:false,content:'<div id = "menuWindow"></div>'});layer.style(menuIndex,{'box-shadow':'none','display':'none'});menuDiv.click(()=>{menuDiv.addClass('active');ContributionList.removeClass('active');GreatNavigation.removeClass('active');layer.style(menuIndex,{'display':'block'});hidePic();});ContributionList.click(()=>{ContributionList.addClass('active')
menuDiv.removeClass('active');GreatNavigation.removeClass('active');layer.style(menuIndex,{'display':'none'});hidePic(false);});GreatNavigation.click(()=>{GreatNavigation.addClass('active')
menuDiv.removeClass('active');ContributionList.removeClass('active');layer.style(menuIndex,{'display':'none'});hidePic(false);});try{BAPI.setCommonArgs(Live_info.bili_jct);}catch(err){console.error(`[${NAME}]`,err);return;}
...
return p},newMessage:(version)=>{try{const cache=localStorage.getItem(`${NAME}_NEWMSG_CACHE`);if((cache==undefined||cache==null||cache!='4.5')&&version=='4.5'){const linkMsg=(msg,link)=>{return'<a href=\"'+link+'\"target=\"_blank\">'+msg+'</a>';};layer.open({title:`${version}更新提示`,content:`<strong>1.界面优化，点击聊天区上方，大航海右侧的【脚本日志】按钮查看部分日志（原来是在聊天区显示）。<br>
...
await sleep(1100);}}},MaterialObject:{list:[],ignore_keyword:['test','encrypt','测试','钓鱼','加密','炸鱼'],run:()=>{try{if(!MY_API.CONFIG.MATERIAL_LOTTERY)return $.Deferred().resolve();if(MY_API.CACHE.materialobject_ts){const diff=ts_ms()-MY_API.CACHE.materialobject_ts;const interval=MY_API.CONFIG.MATERIAL_LOTTERY_CHECK_INTERVAL*60e3||600e3;if(diff<interval){MYDEBUG('[实物抽奖]',`${interval - diff}毫秒后再次运行`);setTimeout(MY_API.MaterialObject.run,interval-diff);return $.Deferred().resolve();}};MY_API.chatLog('[实物抽奖]\n开始寻找可参加的抽奖');return MY_API.MaterialObject.check().then((aid)=>{if(aid){MY_API.CACHE.last_aid=aid;MY_API.CACHE.materialobject_ts=ts_ms();MY_API.saveCache();}
MYDEBUG('[实物抽奖]',`将在${MY_API.CONFIG.MATERIAL_LOTTERY_CHECK_INTERVAL}分钟后再次运行实物抽奖`);setTimeout(MY_API.MaterialObject.run,MY_API.CONFIG.MATERIAL_LOTTERY_CHECK_INTERVAL*60e3||600e3);},()=>delayCall(()=>MY_API.MaterialObject.run()));}catch(err){MY_API.chatLog('[实物抽奖]运行时出现异常','error');console.error(`[${NAME}]`,err);return $.Deferred().reject();}},check:(aid,valid=632,rem=9)=>{aid=parseInt(aid||(MY_API.CACHE.last_aid),10);if(isNaN(aid))aid=valid;MYDEBUG('API.MaterialObject.check: aid=',aid);return BAPI.Lottery.MaterialObject.getStatus(aid).then((response)=>{MYDEBUG('API.MaterialObject.check: API.MY_API.MaterialObject.getStatus',response);if(response.code===0&&response.data){if(MY_API.CONFIG.MATERIAL_LOTTERY_IGNORE_QUESTIONABLE_LOTTERY&&MY_API.MaterialObject.ignore_keyword.some(v=>response.data.title.toLowerCase().indexOf(v)>-1)){MY_API.chatLog(`[实物抽奖]\n忽略抽奖(aid=${aid})`,'info');return MY_API.MaterialObject.check(aid+1,aid);}else{return MY_API.MaterialObject.join(aid,response.data.title,response.data.typeB).then(()=>MY_API.MaterialObject.check(aid+1,aid));}}else if(response.code===-400||response.data==null){if(rem)return MY_API.MaterialObject.check(aid+1,valid,rem-1);return $.Deferred().resolve(valid);}else{MY_API.chatLog(`[实物抽奖]\n${response.msg}`,'warning');}},()=>{MY_API.chatLog(`[实物抽奖]\n检查抽奖(aid=${aid})失败，请检查网络`,'error');return delayCall(()=>MY_API.MaterialObject.check(aid,valid));});},join:(aid,title,typeB,i=0)=>{if(i>=typeB.length)return $.Deferred().resolve();if(MY_API.MaterialObject.list.some(v=>v.aid===aid&&v.number===i+1))
return MY_API.MaterialObject.join(aid,title,typeB,i+1);const number=i+1;const obj={title:title,aid:aid,number:number,status:typeB[i].status,join_start_time:typeB[i].join_start_time,join_end_time:typeB[i].join_end_time};switch(obj.status){case-1:{MY_API.chatLog(`[实物抽奖] 将在${new Date(ts_ms() + obj.join_start_time + 1000).toLocaleString()}参加抽奖"${obj.title}"aid=${obj.aid}`,'success');MY_API.MaterialObject.list.push(obj);const p=$.Deferred();p.then(()=>{return MY_API.MaterialObject.draw(obj);});setTimeout(()=>{p.resolve();},(obj.join_start_time-ts_s()+1)*1e3);}
break;case 0:return MY_API.MaterialObject.draw(obj).then(()=>{return MY_API.MaterialObject.join(aid,title,typeB,i+1);});case 1:{MY_API.chatLog(`[实物抽奖] 已参加抽奖"${obj.title}"aid=${obj.aid}`);MY_API.MaterialObject.list.push(obj);const p=$.Deferred();p.then(()=>{return MY_API.MaterialObject.notice(obj);});setTimeout(()=>{p.resolve();},(obj.join_end_time-ts_s()+1)*1e3);}
break;}
return MY_API.MaterialObject.join(aid,title,typeB,i+1);},draw:(obj)=>{return BAPI.Lottery.MaterialObject.draw(obj.aid,obj.number).then((response)=>{MYDEBUG('API.MaterialObject.check: API.MY_API.MaterialObject.draw',response);if(response.code===0){$.each(MY_API.MaterialObject.list,(i,v)=>{if(v.aid===obj.aid&&v.number===obj.number){v.status=1;MY_API.MaterialObject.list[i]=v;return false;}});const p=$.Deferred();p.then(()=>{return MY_API.MaterialObject.notice(obj);});setTimeout(()=>{p.resolve();},(obj.join_end_time-ts_s()+1)*1e3);}else{MY_API.chatLog(`[实物抽奖] "${obj.title}"(aid=${obj.aid},number=${obj.number})${response.msg}`,'warning');}},()=>{MY_API.chatLog(`[实物抽奖] 参加"${obj.title}"(aid=${obj.aid},number=${obj.number})失败，请检查网络`,'error');return delayCall(()=>MY_API.MaterialObject.draw(obj));});},notice:(obj)=>{return BAPI.Lottery.MaterialObject.getWinnerGroupInfo(obj.aid,obj.number).then((response)=>{MYDEBUG('API.MaterialObject.check: API.MY_API.MaterialObject.getWinnerGroupInfo',response);if(response.code===0){$.each(MY_API.MaterialObject.list,(i,v)=>{if(v.aid===obj.aid&&v.number===obj.number){v.status=3;MY_API.MaterialObject.list[i]=v;return false;}});$.each(response.data.winnerList,(i,v)=>{if(v.uid===Info.uid){MY_API.chatLog(`[实物抽奖] 抽奖"${obj.title}"(aid=${obj.aid},number=${obj.number})获得奖励"${v.giftTitle}"`,'success');return false;}});}else{MY_API.chatLog(`[实物抽奖] 抽奖"${obj.title}"(aid=${obj.aid},number=${obj.number})${response.msg}`,'warning');}},()=>{MY_API.chatLog(`[实物抽奖] 获取抽奖"${obj.title}"(aid=${obj.aid},number=${obj.number})中奖名单失败，请检查网络`,'error');return delayCall(()=>MY_API.MaterialObject.notice(obj));});}}};MY_API.init().then(()=>{try{const promiseInit=$.Deferred();const uniqueCheck=()=>{const t=ts_ms();if(t-MY_API.CACHE.UNIQUE_CHECK>=0&&t-MY_API.CACHE.UNIQUE_CHECK<=15e3){$('.link-toast').remove();$('.igiftMsg').remove();window.toast('有其他直播间页面的脚本正在运行，本页面脚本停止运行','caution');return promiseInit.reject();}else{return promiseInit.resolve();}};uniqueCheck().then(()=>{let timer_unique;const uniqueMark=()=>{timer_unique=setTimeout(uniqueMark,10e3);MY_API.CACHE.UNIQUE_CHECK=Date.now();MY_API.saveCache(false);};window.addEventListener('unload',()=>{if(timer_unique){clearTimeout(timer_unique);MY_API.CACHE.UNIQUE_CHECK=0;MY_API.saveCache();}});uniqueMark();if(parseInt(Live_info.uid)===0||isNaN(parseInt(Live_info.uid))){MY_API.chatLog('未登录，请先登录再使用脚本','warning');return};MY_API.newMessage(GM_info.script.version);MYDEBUG('MY_API.CONFIG',MY_API.CONFIG);StartPlunder(MY_API);})}
