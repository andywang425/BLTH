...
const helpInfo: HelpInfo = {
    ...
    LiveTasks: {
      medalTasks: {
            ...
            h('p', [
              ...
              h('div', [
                ...
                h('ul', [
                  ...
                  h(
                    'li',
                    '熄灭的勋章以完成点亮任务的方式被点亮后，通过完成任务获得的亲密度会被储蓄，且储蓄量存在上限（根据观察上限是100）。需要投喂一个粉丝灯牌才能领取这些亲密度。脚本会在控制台日志中提示储蓄的亲密度量以及是否达到上限。',
                  ),
                ]),
              ]),
            ]),
        ...
        miaoZai: {
          ...
          message: () =>
            h('p', [
              h('div', '完成超能粉丝节活动中粉丝福利→亲密喂养的每日任务，赚取喵崽成长值。'),
              h('div', '对于每个粉丝勋章对应的直播间，脚本会依次执行：'),
              h('ol', [
                h('li', '若还没有选择喵崽，自动选择一只（默认暖系橘猫）。'),
                h('li', '签到，领取猫粮。'),
                h('li', '喂猫，直到猫粮用完。'),
                h('li', '撸猫，直到不再获得成长值。'),
              ]),
              ...
              h('div', [
                h('strong', '注意：'),
                h('ul', [
                  h('li', '该活动为限时活动，活动结束（2026-09-13 23:59:59）后本模块会自动禁用。'),
                  h(
                    'li',
                    '如果想为所有符合黑白名单条件的直播间领取并赠送喵崽的馈赠，可点击下方的【领取并赠送喵崽的馈赠】按钮。',
                  ),
                  h(
                    'li',
                    '喵崽升级时会获得大量亲密度奖励，如果你没有在粉丝勋章熄灭后送过粉丝灯牌，很容易超过亲密度储蓄上限（详见【点亮熄灭勋章】内置说明），建议提前手动投喂粉丝灯牌。',
                  ),
                  h('li', '超能粉丝节活动入口会被【移除直播画面上方杂项】功能移除。'),
                ]),
              ]),
            ]),
        },
        miaoZaiClaimGift: {
          ...
          message: () =>
            h('p', [
              h(
                'div',
                '遍历按黑白名单过滤后的粉丝勋章，对于可领取馈赠的直播间，领取喵崽的馈赠并从礼物包裹中赠送给主播，以获得亲密度。',
              ),
            ]),
        },
      },
  ...
  RemoveElement: {
    ...
    removeHeaderStuff: {
      ...
      message: () =>
        h('p', [
          h('div', '移除直播画面上方各种杂七杂八的东西，比如排行榜、活动轮播图等。'),
          h('div', [h('strong', '注意：'), h('span', '超能粉丝节活动入口会被该功能移除。')]),
        ]),
    },
    removeFlipView: {
      title: '移除礼物栏下方广告',
