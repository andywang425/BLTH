    ...
    LiveTasks: {
      medalTasks: {
        list: {
          title: '黑白名单 / 排序模式',
          message: h('p', [
            h('div', '每个直播任务都有自己独立的黑名单或白名单，互不影响。'),
            h('ul', [
              h('li', [
                h('span', '黑名单：仅为'),
                h('strong', '不在'),
        ...
        like: {
          title: '粉丝团点赞',
          message: h('p', [
            h('div', '为已点亮的粉丝勋章完成粉丝团每日点赞任务。'),
            h('ul', [
              h('li', '只有主播开播时点赞才会计入任务进度，因此只会给开播中的直播间点赞。'),
              h(
                'li',
                '脚本会根据 B 站返回的每日上限自动决定点赞次数，每个等级的勋章上限不同（如 2/2、10/10）。',
              ),
              h('li', '每个直播间分多轮点赞，每轮约 30 次，轮间间隔约 30 秒。'),
            ]),
          ]),
        },
        danmu: {
          title: '粉丝团发弹幕',
          message: h('p', [
            h('div', '为已点亮的粉丝勋章完成粉丝团每日发弹幕任务。'),
            h('ul', [
              h('li', '脚本会根据 B 站返回的每日上限自动决定发送条数，每个等级的勋章上限不同。'),
              h(
                'li',
                '点击【编辑弹幕】按钮编辑要发送的弹幕，脚本会从中按顺序循环抽取弹幕发送。建议修改默认的弹幕。',
              ),
        ...
        watch: {
          ...
          message: h('p', [
            ...
            h('ul', [
              ...
              h(
                'li',
                '脚本会根据 B 站返回的每日上限自动计算观看时长，每观看 15 分钟领取一档亲密度，并附加约 1 分钟缓冲以补偿计时误差。',
              ),
            ]),
            h('div', [
              h('strong', '注意：'),
